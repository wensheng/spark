"""Tool decorator for Spark ADK.
Adapted from Strands SDK.

This module provides the @tool decorator that transforms Python functions into Spark tools with automatic metadata
...
from spark.tools.types import BaseTool, ToolSpec, ToolUse, update_tool_runtime_metadata
...
class FunctionToolMetadata:
    ...
    The extracted metadata is used to generate a tool specification that can be used by Spark Agent to understand and
    validate tool usage.
    """
    ...
    def extract_metadata(self) -> ToolSpec:
        ...
        This method analyzes the function to create a standardized tool specification that Spark Agent can use to
        ...
        tool_spec: ToolSpec = {"name": func_name, "description": description, "parameters": {"json": input_schema}}

        return tool_spec

    def _clean_pydantic_schema(self, schema: dict[str, Any]) -> None:
        """Clean up Pydantic schema to match Spark expected format.

        Pydantic's JSON schema output includes several elements that aren't needed for Spark Agent tools and could
    ...
    def inject_special_parameters(
        ...
        if self._context_param and self._context_param in self.signature.parameters:
            tool_context = ToolContext(tool_use=tool_use, invocation_state=invocation_state)
...
class DecoratedFunctionTool(BaseTool, Generic[P, R]):
    """An BaseTool that wraps a function that was decorated with @tool.

    This class adapts Python functions decorated with @tool to the BaseTool interface. It handles both direct
    ...
    def __init__(
        ...
        *,
        ...
        supports_async_flag = supports_async if supports_async is not None else self._is_async_func

        if supports_async_flag:
    ...
    def __get__(self, instance: Any, obj_type: Optional[Type] = None) -> "DecoratedFunctionTool[P, R]":
        ...
        if instance is not None and not inspect.ismethod(self._tool_func):
            ...
            return DecoratedFunctionTool(
                ...
                tool_func,
                self._metadata,
                long_running=self.is_long_running,
    ...
    def prepare_tool_kwargs(
        self, tool_use: ToolUse, invocation_state: Optional[dict[str, Any]] = None
    ) -> dict[str, Any]:
        """Validate and augment tool_use input for execution."""
        invocation_state = invocation_state or {}
        tool_input = tool_use.get("input") or {}
        if not isinstance(tool_input, dict):
            raise ValueError(f"Decorated tool '{self.tool_name}' expects object input, got {type(tool_input)}")

        validated = self._metadata.validate_input(tool_input)
        self._metadata.inject_special_parameters(validated, tool_use, invocation_state)
        return validated

    async def acall(self, *args: P.args, **kwargs: P.kwargs) -> Any:
        """Async entry point honoring the wrapped function type."""
    ...
    @staticmethod
    def _detect_async_callable(func: Callable[..., Any]) -> bool:
        """Determine whether the callable is coroutine-based."""
...
def tool(  # type: ignore
    ...
    """Decorator that transforms a Python function into a Spark tool.

    This decorator seamlessly enables a function to be called both as a regular Python function and as a Spark tool.
    ...
    4. Formats return values according to the expected Spark tool result format
    ...
    Args:
        ...
        parameters: Optional custom JSON schema to override the automatically generated schema.
        context: When provided, places an object in the designated parameter. If True, the param name
            ...
            the param name.
        long_running: Hint that the tool may take a long time to complete (propagated to metadata/spec).
        async_execution: Override automatic detection of coroutine functions. Set True to force async support
            metadata even for sync callables (useful when you plan to await via acall but wrap sync logic).

    Returns:
        An BaseTool that also mimics the original function when invoked
    ...
    def decorator(f: T) -> "DecoratedFunctionTool[P, R]":
        ...
        return DecoratedFunctionTool(
            ...
            supports_async=async_execution,
        )
