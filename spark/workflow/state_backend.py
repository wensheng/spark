...
from spark.workflow.serializers import StateSerializer, ensure_state_serializer

logger = logging.getLogger(__name__)

WatcherCallback = Callable[[str, Any, Any], Awaitable[None] | None]
_SAFE_SQL_IDENTIFIER = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")
_SAFE_BLOB_ID = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_.-]*$")
...
def _validate_blob_id(blob_id: str) -> str:
    if not _SAFE_BLOB_ID.fullmatch(blob_id) or ".." in blob_id:
        raise ValueError("blob_id must contain only letters, digits, '.', '_', or '-' and cannot contain '..'")
    return blob_id
...
class StateBackend(ABC):
    """Abstract storage backend for graph state."""
...
class InMemoryStateBackend(StateBackend):
    ...
    async def open_blob_writer(
        ...
        *,
        ...
        key = _validate_blob_id(blob_id or uuid4().hex)
        buffer = bytearray()
    ...
    @asynccontextmanager
    async def open_blob_reader(self, blob_id: str) -> AsyncIterator[Any]:
        blob_id = _validate_blob_id(blob_id)
...
class SQLiteStateBackend(StateBackend):
    ...
    def __init__(
        ...
        *,
        ...
        self.table_name = _validate_sql_identifier(table_name, field_name="table_name")
    ...
    async def initialize(self) -> None:
        ...
        self._conn.execute(f"CREATE TABLE IF NOT EXISTS {self.table_name} (key TEXT PRIMARY KEY, value BLOB NOT NULL)")
    ...
    async def _persist_blob_index(self) -> None:
        payload = json.dumps(self._blob_index, indent=2).encode("utf-8")
        await asyncio.to_thread(_atomic_write_bytes, self._blob_index_path, payload)
    ...
    async def open_blob_writer(
        ...
        *,
        ...
        blob_key = _validate_blob_id(blob_id or uuid4().hex)
        await asyncio.to_thread(self._blob_dir.mkdir, parents=True, exist_ok=True)
        fd, tmp_name = tempfile.mkstemp(dir=self._blob_dir, prefix=f".{blob_key}.")
...
class JSONFileStateBackend(StateBackend):
    ...
    async def _write_file(self) -> None:
        ...
        def _write():
            ...
            _atomic_write_bytes(self.path, payload if self._serializer.binary else payload.decode("utf-8").encode())
