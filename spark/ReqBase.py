...
class ReqBase:
	def __init__(self):
		...
		for k in dir(webdir):
			if not k[0:2] == '__':
				self.spark_conf.update({k:getattr(webdir,k)})
	...
	def get_contact(self):
		if self.spark_conf.has_key('rewrites') and self.spark_conf['rewrites']:
			import re
			path = '/'.join(self.path)
			for p in self.spark_conf['rewrites']:
				if re.match(p[0],path):
					if len(p)<2: return ["index","default"]
					elif len(p)<3: return [p[1],"default"]
					else: return [p[1],p[2]]

		if not self.path: return ["index",False]

		return '.'.join(self.path),False
	...
	def run(self):
		...
		#self.data = str([controller,action])
		#self.show()
		#return self.finish()
		
		try: self.con_mod = __import__("webdir."+controller,'','',[''])
		except ImportError: return self.print_exception()

		if not action:
			action = 'default'
			if not hasattr(self.con_mod,action) or  not isinstance(self.con_mod.__dict__[action],FunctionType):
				try:
					self.con_mod = __import__("webdir."+controller+".index",'','',[''])
				except ImportError:
